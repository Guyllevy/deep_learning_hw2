...
class TanH(Layer):
    ...
    def forward(self, x, **kw):
        ...
        #  Save whatever you need into grad_cache.
        # ====== YOUR CODE: ======
        a,b = torch.exp(x), torch.exp(-x)
        out = (a - b)/(a + b)
        self.grad_cache['tanhx'] = out
        # ========================
...
class Linear(Layer):
    ...
    def __init__(self, in_features, out_features, wstd=0.1):
        ...
        # ====== YOUR CODE: ======
        self.w = torch.normal(mean = torch.zeros((self.out_features, self.in_features)), std = wstd)
    ...
    def backward(self, dout):
        ...
        # (N, Din) = (N, Dout) @ (Dout, Din)                                   // dL/dy @ dy/dx
        dx = dout @ self.w 
        
        # (Dout, Din) = (Dout, N) @ (N, Din)                                   // dL/dy @ dy/dw
        self.dw += dout.T @ x
        
        # (Dout) =  sum((N,Dout),0)                                            // dL/dy @ dy/db
...
class CrossEntropyLoss(Layer):
    ...
    def forward(self, x, y):
        ...
        # ====== YOUR CODE: ======
        # x shape (N,num_classes)     y shape (N,)
        num_classes = x.shape[1] 
        
        x_y = x[torch.arange(N),y]
        
        sum_exp_x = torch.sum(torch.exp(x), dim = 1)
        loss_N = - x_y + torch.log(sum_exp_x)
    ...
    def backward(self, dout=1.0):
        ...
        dx = (exp_x / sum_exp_x.unsqueeze(1)) - one_hot
        dx *= 1/N
        dx *= dout 
        # ========================
...
class Dropout(Layer):
    ...
    def forward(self, x, **kw):
        ...
        # ====== YOUR CODE: ======
        self.grad_cache["x_shape"] = x.shape
        if self.training_mode == True:
            rand = torch.rand(x.shape)
            drop_bool = torch.where(rand < self.p, 1.0, 0.0) # 1 for true, means drop.
            out = torch.where(drop_bool > 0.5, torch.zeros_like(x), x)
            out *= 1/(1-self.p)
            self.grad_cache["drop_bool"] = drop_bool
        else:
            out = x
        # ========================
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        if self.training_mode == True:
            drop_bool = self.grad_cache["drop_bool"]
            dx = (1 - drop_bool)
            dx *= 1/(1-self.p)
            dx *= dout
        else:
            dx = torch.ones(self.grad_cache["x_shape"])*dout
        # ========================
...
class MLP(Layer):
    ...
    def __init__(
        ...
        **kw,
        ...
        # ====== YOUR CODE: ======
        if activation != "relu" and activation != "sigmoid":
                raise ValueError("activation is either 'relu' or 'sigmoid'")
        ...
        layers.append(Linear(features[-2], features[-1])) # final fully connected layer
